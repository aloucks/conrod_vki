...
macro_rules! convert_window_event {
    ($event:expr, $window:expr) => {{
        ...
        let (win_w, win_h): (f64, f64) = $window.get_inner_size().unwrap().into();
        ...
        match $event {
            ...
            winit::WindowEvent::Focused(focused) => Some(conrod_core::event::Input::Focus(focused).into()),

            winit::WindowEvent::KeyboardInput { input, .. } => input.virtual_keycode.map(|key| match input.state {
            ...
            winit::WindowEvent::Touch(winit::Touch {
                ...
                let touch = conrod_core::input::Touch { phase, id, xy };
            ...
            winit::WindowEvent::MouseWheel { delta, .. } => match delta {
                winit::MouseScrollDelta::PixelDelta(winit::dpi::LogicalPosition { x, y }) => {
                    ...
                    let motion = conrod_core::input::Motion::Scroll { x, y };
                    Some(conrod_core::event::Input::Motion(motion).into())
                }
                ...
                winit::MouseScrollDelta::LineDelta(x, y) => {
                    ...
                    Some(conrod_core::event::Input::Motion(conrod_core::input::Motion::Scroll { x, y }).into())
                }
            },
            ...
            winit::WindowEvent::MouseInput { state, button, .. } => match state {
                ...
                winit::ElementState::Released => Some(
                    conrod_core::event::Input::Release(conrod_core::input::Button::Mouse(map_mouse(button))).into(),
                ),
            },

            winit::WindowEvent::Refresh => Some(conrod_core::event::Input::Redraw),
...
macro_rules! conversion_fns {
    () => {
        ...
        pub fn convert_mouse_button(mouse_button: winit::MouseButton) -> conrod_core::input::MouseButton {
            convert_mouse_button!(mouse_button)
        }

        /// Convert a given conrod mouse cursor to the corresponding winit cursor type.
        pub fn convert_mouse_cursor(cursor: conrod_core::cursor::MouseCursor) -> winit::MouseCursor {
            convert_mouse_cursor!(cursor)
        }

        /// A function for converting a `winit::WindowEvent` to a `conrod_core::event::Input`.
        pub fn convert_window_event<W>(event: winit::WindowEvent, window: &W) -> Option<conrod_core::event::Input>
        where
            W: $crate::WinitWindow,
        {
        ...
        pub fn convert_event<W>(event: winit::Event, window: &W) -> Option<conrod_core::event::Input>
        where
            W: $crate::WinitWindow,
        {
